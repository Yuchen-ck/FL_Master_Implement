...
def argparse_():
    ...
    local_train_bs = 1000
    local_defense_bs = server_defense_bs = 128 # train_bs和defense_bs一致，因為要加起來一起訓練
    local_test_bs = 16
    model_name = 'MNIST_Net_paper'
    optimizer ='Adam'
    lr = 0.01

    # 3. Local_Train Algo setting
    FedProx = 0  # 0:FedAvg
    ...
    parser.add_argument('--pri', type=float, default= p, help='weight for (1+alpha)/(1-alpha): 1.2, 1.4, 1.6 ...')
    ...
    # 論文固定的eps
    mnist_eps = 0.5
    parser.add_argument('--mnist_eps', type=float, default= mnist_eps, help=' c2: balance factor  - 0.01')
    parser.add_argument('--alpha', type=float, default= 0.375, help=' c2: balance factor  - 0.01')

    defense_data_numbers = 300 #(3000/60000 = 0.05) (5%資料放進server)
    test_numbers = 16
